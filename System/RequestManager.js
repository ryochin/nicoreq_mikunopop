...
//				beforeValue = "[" + RequestManager.Requests[id].tags.join("][") + "]";
	...
	getItemHTML: function(RQ){
		...
		ItemHTML += "<br><input type=\"image\" src=\"./System/assets/remove.png\" class=\"control-button-down delete-button\" onclick=\"RequestManager.deleteRequestQueueById('{#ID}')\" value=\"削除\"></td>";
		ItemHTML += "</td><td>";
	...
	_replaceHTML: function(str, R){
		...
		if( R.no_live_play == 1 ){
			...
			thumb_url = "<img src=\"" + thumb_dummy_path + "\" width=65 height=50 align=left title=\"" + thumb_title + "\" oncontextmenu=\"" + oncontextmenu + "\" class=\"thumb\" />";
		}
		else if( NGIDs[R.id] == true ){
			// NGID に登録されている（主セレの場合）
			var thumb_dummy_path = location.href.toString().replace(/NicoRequest\.hta$/,"")

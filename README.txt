...
2009.07.02 1.03 リクエストをリストの上部に追加する機能を追加
　・[実験的] リスナーからのリクエストを、リクエストリストの
　　末尾ではなく先頭（上）に追加するオプションを追加しました。
　　settings.jsの８２行目付近「リクエスト追加順」の項を参照してください。
　　※デフォルトの挙動が変更されています！
　　　注意１：このオプションは予期せぬ誤動作を招く恐れがあります。
　　　注意２：強引な実装なので、内部的な並び順の整合性が取れていません。
　・P名のリストを別ファイル（pnames.js）へ分離しました。
　　随時追加しています。情報もお待ちしています。
